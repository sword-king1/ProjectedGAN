...
def _make_efficientnet(model):
    ...
    pretrained.layer0 = nn.Sequential(model.conv_stem, model.bn1, model.act1, *model.blocks[0:2])#0-1
    pretrained.layer1 = nn.Sequential(*model.blocks[2:3])#2
    pretrained.layer2 = nn.Sequential(*model.blocks[3:5])#3-4
    pretrained.layer3 = nn.Sequential(*model.blocks[5:9])#5-8
    return pretrained
...
#cout=64 im_res=256 proj_type=2
def _make_projector(im_res, cout, proj_type, expand=False):
    ...
    #设置四个特征图的channels大小 也就是CCM的输入 x*y*Z 中的Z
