...
def training_loop(
    ...
    data_loader = torch.utils.data.DataLoader(dataset=training_set, sampler=training_set_sampler, batch_size=batch_size // num_gpus,
                                **data_loader_kwargs)
    ...
    # opt_0 = dnnlib.util.construct_class_by_name(params=D.discriminator.mini_discs["0"].parameters(),D_opt_kwargs)  # subclass of torch.optim.Optimizer
    # opt_1 = dnnlib.util.construct_class_by_name(params=D.discriminator.mini_discs["1"].parameters(),D_opt_kwargs)  # subclass of torch.optim.Optimizer
    # opt_2 = dnnlib.util.construct_class_by_name(params=D.discriminator.mini_discs["2"].parameters(),D_opt_kwargs)  # subclass of torch.optim.Optimizer
    # opt_3 = dnnlib.util.construct_class_by_name(params=D.discriminator.mini_discs["3"].parameters(),D_opt_kwargs)  # subclass of torch.optim.Optimizer
    print("提前注册opt")
    ...
    for name, module, opt_kwargs, reg_interval in [('G', G, G_opt_kwargs, G_reg_interval), ('D', D, D_opt_kwargs, D_reg_interval)]:
        ...
        if module == D:
            if reg_interval is None:
                opt0 = dnnlib.util.construct_class_by_name(params=module.discriminator.mini_discs["0"].parameters(),
                                                           **opt_kwargs)  # subclass of torch.optim.Optimizer
                opt1 = dnnlib.util.construct_class_by_name(params=module.discriminator.mini_discs["1"].parameters(),
                                                           **opt_kwargs)  # subclass of torch.optim.Optimizer
                opt2 = dnnlib.util.construct_class_by_name(params=module.discriminator.mini_discs["2"].parameters(),
                                                           **opt_kwargs)  # subclass of torch.optim.Optimizer
                opt3 = dnnlib.util.construct_class_by_name(params=module.discriminator.mini_discs["3"].parameters(),
                                                           **opt_kwargs)  # subclass of torch.optim.Optimizer
                opt = dnnlib.util.construct_class_by_name(params=module.parameters(), **opt_kwargs) # subclass of torch.optim.Optimizer
                D_0, opt0, data_loader2 = privacy_engine.make_private(
                    module=D.discriminator.mini_discs["0"],
                    optimizer=opt0,
                ...
                print("privacy->D1")
                D_1, opt1, data_loader2 = privacy_engine.make_private(
                    module=D.discriminator.mini_discs["1"],
                    optimizer=opt1,
                    data_loader=data_loader,
                    noise_multiplier=1.1,
                    max_grad_norm=1.0,
                ...
                print("privacy->D2")
                D_2, opt2, data_loader2 = privacy_engine.make_private(
                    module=D.discriminator.mini_discs["2"],
                    optimizer=opt2,
                    data_loader=data_loader,
                    noise_multiplier=1.1,
                    max_grad_norm=1.0,
                ...
                print("privacy->D3")
                D_3, opt3, data_loader2 = privacy_engine.make_private(
                    module=D.discriminator.mini_discs["3"],
                    optimizer=opt3,
                    data_loader=data_loader,
                    noise_multiplier=1.1,
                    max_grad_norm=1.0,
                ...
                print("privacy->D4")
                D.discriminator.mini_discs["0"] = D_0
                D.discriminator.mini_discs["1"] = D_1
                D.discriminator.mini_discs["2"] = D_2
                D.discriminator.mini_discs["3"] = D_3
                opt_0 = opt0
                opt_1 = opt1
                opt_2 = opt2
                opt_3 = opt3
                ...
                print("type==1", type(data_loader2))
            else: # Lazy regularization.
                mb_ratio = reg_interval / (reg_interval + 1)
                opt_kwargs = dnnlib.EasyDict(opt_kwargs)
                opt_kwargs.lr = opt_kwargs.lr * mb_ratio
                ...
                opt_2 = opt2
                opt_3 = opt3
                print("type==2", type(data_loader2))
                phases += [dnnlib.EasyDict(name=name+'main', module=module, opt=opt, interval=1)]
                phases += [dnnlib.EasyDict(name=name+'reg', module=module, opt=opt, interval=reg_interval)]
                data_loader = data_loader2
    print(type(opt_0), "opt_0的type（）")
    training_set_iterator = iter(data_loader)
    for phase in phases:
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            print("# Accumulate gradients.-------1")
            if phase.name in ['Dmain', 'Dboth', 'Dreg']:
                print("zero_grad(set_to_none=True)")
            ...
            for real_img, real_c, gen_z, gen_c in zip(phase_real_img, phase_real_c, phase_gen_z, phase_gen_c):
                ...
                print("time : loss.accumulate_gradients", b , "----------2.0")
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, real_c=real_c, gen_z=gen_z, gen_c=gen_c, gain=phase.interval, cur_nimg=cur_nimg)
            print("#phase.module.requires_grad_(False)")
            phase.module.requires_grad_(False)

            # Update weights.
            print("# Update weights.------------------------3")
